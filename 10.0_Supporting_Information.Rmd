...
```{r erd-mort-census, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "100%", fig.pos = 'H', results = "asis", fig.cap = "\\label{lab-erd-mort-census}Species mortality rate in each census interval during the study period plotted against ERD. $R$^2^ and significance levels for linear model fits are given in panel insets. Linear model fits (blue lines) with confidance bands (gray area) are shown for significant fits at $\\alpha$ = 0.05, identified by census-intervals with a superscript *."}
