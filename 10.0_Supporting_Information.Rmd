...
```{r swp-all-depths, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "70%", fig.pos = 'H', results = "asis", fig.cap = "\\label{lab-swp-all-depths}Daily mean soil water potential ($\\Psi_{\\textrm{soil},z}$) by depth $z$ (m; panels) over 1990-2018 predicted from each of the the 100 best-fit ensembles (colored lines). Vertical gray lines demarcate median dates of the demographic censuses at the BCI 50 ha plot. Years 1991 & 1992 for depths > 2.9 m showed marks of model initialisation phase for ELM-FATES, which was run from ($\\Psi_{\\textrm{soil},z}$) = 0 for all depths, starting from 1985-01-01. We therefore removed $\\Psi_{\\textrm{soil}}$ for years 1991 & 1992 for depths greater than 2.9 m, and conservatively removed $\\Psi_{\\textrm{soil}}$ for year 1991 for the depth of 2.9 m."}
...
```{r erd-mort-census, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "100%", fig.pos = 'H', results = "asis", fig.cap = "\\label{lab-erd-mort-census}Species (circles) Mortality rates in species of various deciduous leaf-habit categories (colors) across census intervals from 1985-2015 plotted against ERD. $R$^2^ and significance levels for linear model fits are given in panel insets. Species are color colded by deciduous category. Census-intervals with a superscript * are shown for comparison and indicate intervals in which significant mortality was explained by ERD among evergreen species (Fig. 8 in the Main text)."}
...
\
For five species, an estimate of leaf lifespan was available from direct observations [@Osnas.2018]. For the rest `r erd.sp.wo.ll` species, leaf lifespan was predicted from LMA ($\overline{lifespan} = `r ll.lma.cf[1]` + `r ll.lma.cf[2]`LMA$; $R$^2^ = `r ll.lma.r2`; $p$ `r ll.lma.p`, $n$ = `r ll.lma.n`).


```{r leaf-cover, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "50%", fig.pos = 'H', results = "asis", fig.cap = paste0("\\label{lab-leaf-cover}Seasonal variation in standardised LAI (range 0-1) for ", length(erd.sp)," species at Barro Colorado Island (panels). Species are color-coded by deciduous category as defined by expert botanists.")}
