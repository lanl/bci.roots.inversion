...
LAI.sp <- lapply(lapply(leaf.fall.sp, leaf.cover_fun), as.data.frame) %>%
  bind_rows(.id = "sp") %>%
  mutate(doy = as.numeric(format(date, "%j")),
         year = as.numeric(format(date, "%Y"))) %>%
  group_by(sp) %>%
  mutate(LAI = leaf_cover/LMA) %>%
  ungroup(sp) %>%
  group_by(sp, doy) %>%
