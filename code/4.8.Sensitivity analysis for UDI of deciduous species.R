...
for (j in 1: length(level.folder)){
  ...
  names(min.ll.list[[j]]) <- drop.months.vec
...
heat.udi <- ggplot(min.ll %>% subset(size == "large") %>% droplevels(),
                  aes(y = deciduous_sp, x = as.factor(drop.months))) +
  ylab("Species") + xlab("Months Dropped from Btran Calculation") +
  facet_wrap(. ~ tlplevel) +
...
top.udi.ll <- heat.udi + geom_tile(aes(fill = udi.med.ll)) +
  ggtitle("Median UDI for top ranking models with minimum negative Log Likelihood")
ggsave("med.top.UDI_within_sp_across_drop.months_min.negLL.jpeg", plot = top.udi.ll, path =
         file.path("figures/UDI_confidence/sensitivity_analysis"), height = 8.94, width = 8.94, units='in')

top.udi.ll.upr <- heat.udi + geom_tile(aes(fill = udi.upr.ll)) +
  ggtitle("97.5 quantile UDI for top ranking models with minimum negative Log Likelihood")
ggsave("upr.top.UDI_within_sp_across_drop.months_min.negLL.jpeg", plot = top.udi.ll.upr, path =
         file.path("figures/UDI_confidence/sensitivity_analysis"), height = 8.94, width = 8.94, units='in')

top.udi.rsq <- heat.udi + geom_tile(aes(fill = udi.med.rsq)) +
  ggtitle("Median UDI for top ranking models with maximum R-squared")
