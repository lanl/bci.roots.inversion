...
We test an overarching hypothesis that interspecific variation in effective rooting depths of trees aligns them on a conservative-acquisitive trade-off axis. Specifically, in this forest (i) dynamics of soil water potential at depth does not cross catastrophic levels even during multi-annual droughts because of high total annual precipitation to evapotranspiration ratio; (ii) trees with deeper effective rooting depths (ERD) show acquisitive hydraulic architecture: higher maximum stem hydraulic conductance, but greater xylem vulnerability to embolism and narrower hydraulic safety margins; finally, (iii) deeper ERD species have lower mortality rates due to low realised risk of hydraulic failure. To our knowledge this is the first study to demonstrate a mechanistic link between trees' realised drought experience, above-ground hydraulic architecture and mortality outcomes for a tropical forest community.
...
Growth Data: To minimise effect of light variation among trees and census intervals we selected only large trees (>= 30 cm DBH) and species whose maximum height was at least 30 m (hereafter, canopy species). We chose only those trees for which diameter (DBH) record was present across all size censuses (1990, 19905, 2000, 2005, 2010, 2015). We calculated individual tree ($i$) growth rates ($Gr_{i,t}$ cmyr^{-1}) for each of the five census intervals ($t$). As tree growth rates increase with size, to keep growth rates comparable across census intervals, we obtained residuals from a DBH model of tree growth based on a trees' DBH at the beginning of a census onterval ($GrResid_{i,t} = Gr_{i,t} - f(DBH_{i, t})$). The diameter model of growth was fitted based on all trees' data pooled together as there was insufficient species-specific data for large trees (SI ). We then standardised (centered and scaled) an individual tree time series ($GrResid_{i,t}$) and obtained species-level (*n* = 32) times-series of medians ($Growth_{t}$) that had data on at least three trees (median = xx). 
...
For each model (\@ref(eq:M2)- \@ref(eq:M4)) and species combination ($sp$), we obtained goodness-of-fit (*R*^2^) and Pearson's $\rho$ for all soil depths $z$, and for each sp, model we chose the $z$ with highest *R*^2^ as the Effective Rooting Depth (ERD); after retaining species with positive $\rho$ and *R*^2^ >= 0.1.
...
Models with VPD and without Leaf Fraction (Eqs. \@ref(eq:M2)) had highest goodness-of fit (*R*^2^ = 0.51); followed by model with both VPD and Leaf Fraction (Eq. \@ref(eq:M4); Fig. S\@ref(fig:other-erd-mods)). Models with leaf fraction had only three species with sufficient, indvidual species-level goodness-of-fit (Eqs. \@ref(eq:M3 & eq:M4) versus seven for models without (Eqs. \@ref(eq:M1) & \@ref(eq:M2)). For rest of the analyses we therefore used species ERD estimates from model (\@ref(eq:M2)) as it was also a parsimonious model compared to \@ref(eq:M4) (Fig. \@ref(fig:erd-iso)).
