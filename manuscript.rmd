...
To estimate effective rooting depth, we tested alternate models for growth (Eq. \@ref(eq:M-chosen) and those in SI), which use effective rooting depth as an input. There are many intrinsic and extrinsic factors that limit plant growth, including soil moisture, VPD, leaf area seasonality and radiation [@Lawrence.2019]. Rhizosphere soil water potential ($\mathrm{\Psi}$~soil~) down-regulates plant tissue water potentials [@Christoffersen:2016kd2d]. In moist forests $\mathrm{\Psi}$~leaf~ equilibrates with $\mathrm{\Psi}$~soil~ at night and declines through the day as plants transpire. $\mathrm{\Psi}$~leaf~ in turn controls leaf conductance ($K_\textrm{leaf}$), which in turn controls stomatal conductance and photosynthetic rates [@Sack.2012]. We therefore used maximum diurnal leaf conductance ($K_\textrm{leaf}$) as determined by $\mathrm{\Psi}$~soil~ as a proxy for the soil water limitation on growth. In our ERD models $K_\textrm{leaf}$ therefore incorporates the effects of rooting depth and rhizosphere water potential. This approach does not specifically incorporate the buffering effect of stem water storage on $K_\textrm{leaf}$, which may cause the model to slightly overestimate ERD (see Discussion). VPD has a positive impact on transpiration and thus photosynthesis and growth upto a VPD threshold, after which VPD has a negative impact [@Grossiord.2020]. The fraction of a tree's leaf cover present at a given time further limits it's growth potential. We therefore regressed species-specific growth in a census interval against census interval means of multiplicative or additive effects of one or more factors calculated daily: VPD, $K_\textrm{leaf}$ and the leaf area index (LAI), all standardized between 0 and 1. Based on our preliminary analyses, additional incorporation of radiation did not improve model fitting. At our study site, due to the coincidence of higher temperature, higher radiation, and lower humidity during the dry season, the impact of VPD may as well be a combined impact of not only temperature and humidity, but also radiation.
...
\
\    To account for the non-linear impact of VPD on growth, we used a locally derived polynomial relationship between gross primary productivity (GPP) and VPD (see Model parametarisation) and used GPP standardised between 0 and 1 (hereafter, VPD*) in Eq. \@ref(eq:M-chosen).
...
We fitted a polynomial curve between monthly average of mean midday values (11:00 to 15:00 hrs) of gross primary productivity (gC m^2^ day^-1^) we collected from the eddy flux tower located 75 m from the 50-ha plot (2012-2017) and VPD (kPa) calculated using the climate forcing data (for any day $i$, $\textrm{GPP}_i = `r vpd.cf[1]` + `r vpd.cf[2]`\textrm{VPD}_i + `r vpd.cf[3]`\textrm{VPD}_i^2 + error_{i}$; Fig. S\@ref(fig:gpp-vpd)); used VPD to predict GPP from 1990-2015, standardized it between 0 and 1, and used it as $\textrm{VPD}^*$ in Eq. \@ref(eq:M-chosen)).

### Model validation and selection

Xylem sap $\mathrm{\delta}$^2^H values (hereafter, $\mathrm{\delta}$^2^H~xylem~) can provide an index of rooting depth, which we used to validate the ERD models as an independent observation of ERD. We used species $\mathrm{\delta}$^2^H~xylem~ values from BCI given in @Meinzer:2001 for the dry-season of March 1997 as this period showed largest seasonal divergence among species, with a corresponding vertical gradient of $\mathrm{\delta}$^2^H in the soil (hereafter, $\mathrm{\delta}$^2^H~soil~ ). (Fig. S\@ref(fig:iso-data)). Briefly, this isotopic approach makes use of a naturally formed gradient in deuterium to hydrogen ratio of soil water vertically belowground, which is generally formed due to evaporative enrichment (here, during the dry season). Shallow soil water has a greater deuterium to hydrogen content than deep soil water (here, $\mathrm{\delta}$^2^H~soil~ ranged from -16.5 to -50 ‰ over the depth of 0.5-1 m), with ground-water having the lowest deuterium to hydrogen content (here, -60 ‰), typically reflecting un-fractionated rain water. As root water-uptake is generally a non-fractionating process, $\mathrm{\delta}$^2^H~xylem~ of a tree reflects the signature of the source water. Thus water-sourcing depth of the tree can be identified as the depth with the same value of $\mathrm{\delta}$^2^H~soil~, or as that of $\mathrm{\delta}$^2^H~xylem~, with the uncertainty that the value of $\mathrm{\delta}$^2^H~xylem~ may also result from a mixing of water from shallower or deeper depths [@Dawson.1991]. Analyses of $\mathrm{\delta}$^2^H~xylem~ of the 12 species studied by @Meinzer:2001 showed that four species sourced water between 1 m and ground water. Two other had water access at least 0.8 m deep, one was restricted to water access between 0 to 0.2 m and five fell in the range over which the relationship between soil depth and $\mathrm{\delta}$^2^H~soil~ was variable in the 0.2 to 0.7 m depth range. 

\   We compared six species with data for $\mathrm{\delta}$^2^H~xylem~ as well as modelled estimates of ERD, after removing those species in @Meinzer:2001 that are likely to have no leaves in March 1997 because xylem sap $\mathrm{\delta}$^2^H of these species may not be linked with water sourced at the time of measurement and thus may not be comparable with other species that had leaves. We also removed *Guapira standleyana*, whose xylem sap isotope value was not well constrained by the soil water isotope depth profile. For each model of ERD (Eq. \@ref(eq:M-chosen) and Eqs. in SI), estimated species ERD was regressed against species $\mathrm{\delta}$^2^H~xylem~ (‰).
