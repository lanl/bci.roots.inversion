...
- Among the mechanisms that plants employ to tolerate drought, deep water access is arguably the most commonly invoked, but under-studied. Vulnerability to embolism and hydraulic safety margins can predict mortality risk at given levels of dehydration, but deep-water access may delay plants from dehydration. Here, we tested the role of deep-water access in enabling survival within a diverse tropical forest community (Panama) using a novel data-model approach. 
...
\
\   We developed a novel, empirical inverse model to estimate species effective rooting depths (ERD) for large trees of `r length(erd.sp)` species (Fig. \@ref(fig:schematic)). We defined species ERD as the depth at which soil water potential best explained species-specific 5-yrly mean growth rates over 25 years. To obtain the daily dynamics of soil water potential in the whole-soil column we locally parameterized and calibrated a 1-D hydrological water balance model. The ERD model incorporated the impact of atmospheric drought on growth and was constrained by above-ground hydraulic traits, namely, species-specific leaf vulnerability curves, by using existing data for eight species and developing trait-based proxies for the rest. We evaluated ERD against existing dueterium to hydrogen content data for a subset of six tree species as an independent observation of ERD. We tested relationship of ERD with above-ground hydraulic traits, namely, xylem vulnerability to embolism, branch hydraulic safety margins, leaf turgor loss point and maximum stem-specific hydraulic conductivity, for a subset of eight species with both datasets. Finally, we analysed species' mortality rates over 35 years to test whether ERD explained risk of drought-induced mortality in periods of extreme hydrological drought.  
\
...
(\#eq:water-balance)
\end{equation}
where, $q_{\textrm{rain}}$ is rainfall, $E_{v}$ is transpiration of the forest, $E_{g}$ is ground evaporation, $q_{\textrm{over}}$ is surface runoff, $q_{\textrm{h2osfc}}$ is runoff from surface water storage, $q$~drai~ is sub-surface drainage (all in mm H~2~O) and $\mathrm{\Delta}t$ is the time-step (typically, 1 hour). The soil profile is discretized into 15 exponentially distributed soil layers with depth $z$, where $z$ $\in$ $Z$, and $Z$ = {0.01, 0.03, 0.06, 0.12, 0.21, 0.37, 0.62, 1, 1.7, 2.9, 4.7, 7.8, 13, 21, 35} m.

### Model parameterization

To parameterize ELM-FATES, we identified eleven parameters relevant for Eq. \@ref(eq:water-balance), determined their ranges based on literature for the study site, when available, else for the tropics (Table \@ref(tab:param)), and ran 5000 simulations using Latin Hypercube Sampling (LHS) of this global parameter space. In the Conrad catchment, existing data on soil hydraulic conductivity for the depths of 12.5 and 60 cm showed a decline of conductivity by depth (@Kinner:2004; Table \@ref(tab:param)). We generated 5000 LHS samples within the 95% CI for the two depths and assumed a linear decline between each paired sample, while for depths < 12.5 cm and > 60 cm, we assumed the same value as that for depths 12.5 and 60 cm, respectively (Fig. S\@ref(fig:ksat-profiles) [Need to fix SI Figs. numbering to begin at 1]). For greater accuracy, instead of the ELM default soil texture-based pedo-transfer functions, we estimated parameters of soil retention curves using existing data for gravimetric water content vs. $\mathrm{\Psi}$~soil~ for depths of 0.15, 0.4 and 1 m [@Kupers:2019]. We converted gravimetric water content to volumetric (VWC, $\theta$) using a bulk density value of of 0.8 g cm^-3^ measured at a point location (see below). A @Campbell.1974 empirical equation was fitted for all depths pooled together, as exploratory analyses indicated differences in water retention among depths were nominal,
...
We ran ELM-FATES with the chosen 100 ensemble members from 1985-2018, with the first 5 years used for model spin-up. Extreme soil droughts were identified by depth $z$ as days that fell below the 5th percentile of $\mathrm{\Psi}_{\textrm{soil},z}$ for a given day of the year.

## Model for effective rooting depth (ERD)

To estimate effective rooting depth, we tested alternate models for growth (Eq. \@ref(eq:M-chosen) and those in SI), which use effective rooting depth as an input. There are many intrinsic and extrinsic factors that limit plant growth, including soil moisture, VPD, leaf area seasonality and radiation [@Lawrence.2019]. Rhizosphere soil water potential ($\mathrm{\Psi}$~soil~) down-regulates plant tissue water potentials [@Christoffersen:2016kd2d]. In moist forests $\mathrm{\Psi}$~leaf~ equilibrates with $\mathrm{\Psi}$~soil~ at night declining through the day as plants transpire. $\mathrm{\Psi}$~leaf~ in turn controls leaf conductance ($K_\textrm{leaf}$), which can in turn control transpiration and photosynthetic rates [@Sack.2012]. We therefore used maximum diurnal ($K_\textrm{leaf}$) as determined by $\mathrm{\Psi}$~soil~ as a proxy for the soil water limitation on growth, and thus $K_\textrm{leaf}$ in our models incorporates the effects of rooting depth and rhizosphere water potential. This does not specifically account for stem water storage capacitance, which may cause the model to slightly overestimate ERD (see Discussion). Transpiration increases with VPD upto a VPD threshold, after which VPD has a negative impact as leaf pores (stomata) close, reducing photosynthesis and growth  [@Grossiord.2020]. Apart from stomatal control leaf-drop may further limit water-uptake and transpiration. We therefore regressed species-specific growth rates from 5-yrly censuses against census interval means of multiplicative or additive effects of one or more factors calculated daily: VPD, $K_\textrm{leaf}$ and the leaf area index (LAI), all standardized between 0 and 1. Based on our preliminary analyses, additional incorporation of radiation did not improve model fitting. At our study site, due to the coincidence of higher temperature, higher radiation, and lower humidity during the dry season, the impact of VPD may as well be a combined impact of not only temperature and humidity, but also radiation.
...
\
\    To account for the non-linear impact of VPD on growth, we used a locally derived polynomial relationship between gross primary productivity (GPP) and VPD (see SI) and used GPP standardised between 0 and 1 (hereafter, VPD*) in Eq. \@ref(eq:M-chosen).
...
(\#eq:erd)
\end{equation}
where $L$ is the likelihood, $H$ = {1, .., 100}, refer to the 100 hydrological realisations and $Z$ = {0.4, `r paste(soil.depths[8:length(soil.depths)], collapse=", ")`} m. We removed finely resolved soil layers below 0.21 m, averaged $\mathrm{\Psi}_{\textrm{soil},z}$ for depths 0.21, 0.37 and 0.62 at 0.4 m. 

### Model parameterization

#### Growth data

For growth estimates, to minimize the effect of light variation among trees, we selected only large trees ($\ge$ 30 cm dbh, the diameter at breast height) and also species whose maximum height was $\ge$ 30 m (hereafter, canopy species) and thus are likely to be fully exposed to the sun. We calculated individual tree diameter growth rates (cm yr^-1^) across six censuses (1990, 1995, 2000, 2005, 2010, 2015), as the difference between tree dbh measured prorated by exact date of measurements. After removing growth outliers due to measurement error, to account for temporal changes in growth rates due to increasing tree size, we obtained residuals from a dbh model of tree growth. (see SI for details). Retaining trees with a complete record in all censuses (total trees = `r sum(g.n$n)`) and centering and scaling residuals of each tree, we obtained species-level median time series of residuals ($G_{t}$) used in the ERD model, for only those species ($n$ = `r length(erd.sp)`) with complete records on at least three trees (median $n$ = `r median(g.n$n)`, max $n$ = `r max(g.n$n)` trees per species).

#### Leaf hydraulic vulnerability curves

We obtained leaf hydraulic vulnerability curves ($K_\textrm{leaf}$ vs. $\mathrm{\Psi}$~leaf~; see Table \@ref(tab:symbols) for trait symbols and their definitions) for adult trees of 21 common species from Zailaa et al. (to be submitted; see SI for brief description of methods) described as:
...
In some of the alternative models for effective rooting depth, we explored the effect of seasonality in leaf area index (LAI, unitless) on growth. We assumed species-specific mean seasonal curves for LAI normalised between 0 and 1 (LAI, unitless), informed by a combination of long-term records for leaf-fall [@Wright.1990aeo] and leaf-lifetime [@Osnas.2018] (see SI). 

### Model validation and selection
\
As root water-uptake is generally a non-fractionating process, 2^H/1^H content of the tree xylem sap (hereafter, $\mathrm{\delta}$^2^H~xylem~) reflects the signature of the source water. Given a vertical gradient in $\mathrm{\delta}$^2^H water by depth, $\mathrm{\delta}$^2^H~xylem~ can be used as an index of rooting depth [@Dawson.1991]. We thus used $\mathrm{\delta}$^2^H~xylem~ as an independent observation to validate the ERD models. We leveraged $\mathrm{\delta}$^2^H~xylem~ values from BCI given in @Meinzer:2001 for the dry-season of March 1997 as this period showed largest seasonal divergence in $\mathrm{\delta}$^2^H~xylem~ among species, in soil and ground water $\mathrm{\delta}$^2^H at natural abundance level (Fig. S\@ref(fig:iso-data)). Analyses of $\mathrm{\delta}$^2^H~xylem~ of the 12 species studied by @Meinzer:2001 showed that four species sourced water between 1 m and ground water, two other at least 0.8 m deep, one between 0 to 0.2 m, and five in the 0.2 to 0.7 m depth range over which the relationship between soil depth and $\mathrm{\delta}$^2^H~soil~ was variable. 
\
\   We removed those species that would be leafless in March 1997 because xylem sap $\mathrm{\delta}$^2^H for these species may not be linked with water sourced at the time of measurement and thus may not be comparable with other species that had leaves. We also removed one species whose $\mathrm{\delta}$^2^H~xylem~ was in the uncertain depth range. For each model of ERD (Eq. \@ref(eq:M-chosen) and Eqs. in SI), estimated species ERD was regressed against species $\mathrm{\delta}$^2^H~xylem~, for a maximum of six species.
...
For the subset of eight species with ERD for which hydraulic traits data were available, species with deeper ERD showed greater maximum stem hydraulic conductance ($K_\textrm{max,stem}$, Spearmans' $r$ = `r cor.k`, $p$ = `r p.k`; Fig. \@ref(fig:erd-hyd)A), less negative leaf $\mathrm{\Psi}$~tlp~ (Spearmans' $r$ = `r cor.tlp`, $p$ = `r p.tlp`; Fig. \@ref(fig:erd-hyd)B), less negative $\mathrm{\Psi}$~88,stem~ and thus greater vulnerability to embolism from cavitation (Spearmans' $r$ = `r cor.stem.88`, $p$ = `r p.stem.88`; Fig. \@ref(fig:erd-hyd)C), and narrower hydraulic safety margins ($\mathrm{\Psi}$~min~ - $\mathrm{\Psi}$~88,stem~; Spearmans' $r$ = `r cor.hsm`, $p$ = `r p.hsm`; Fig. \@ref(fig:erd-hyd)D). 

Fig. \@ref(fig:erd-hyd) here.

## Effective rooting depths, experienced drought and mortality

Amongst the evergreen species ERD explained `r m.evg.r2.90`, `r m.evg.r2.05` and `r m.evg.r2.10`% variation in mortality rates during census intervals 1985-1990, 2000-2005 and 2005-2010, respectively, such that species with deeper ERD had significantly lower mortality rates than deeper ERD species ($p$ = `r m.evg.p.90`, `r m.evg.p.05` and `r m.evg.p.10`, respectively; $n$ = `r nrow(mrate.depth.mean %>% subset(deciduous == "E"))`; Fig. \@ref(fig:erd-mort-census-evg)). When deciduous species were included, ERD explained lower but significant proportion of variation in mortality in those intervals ($R$^2^ = `r m.r2[1]`, `r m.r2[2]` and `r m.r2[3]`% and $p$ = `r m.p.90`, `r m.p.05` and `r m.p.10`, respectively; $n$ = `r nrow(mrate.depth.mean)`; Fig. \@ref(fig:erd-mort-census)). Census intervals 2000-2005 and 2005-2010 were also the periods during which extreme soil droughts distinctly occurred in the wet season, effectively extending the dry season into the wet season (Fig. \@ref(fig:swp-droughts)). Soil water availability was not modelled for 1985-1990. Analysis of a species' time spent under significant soil drought (proportion of days $\mathrm{\Psi}_{\textrm{soil},z=\textrm{ERD}}$ < $\mathrm{\Psi}$~crit~) showed that species with shallower ERD spent greater proportion of days below critical hydraulic thresholds (Fig. \@ref(fig:erd-time); S\@ref(fig:erd-time-evg)). Species with ERD deeper than 1 m did not cross species $\mathrm{\Psi}$~crit~ for the entire 25-yr period for which soil water potential $\mathrm{\Psi}_{\textrm{soil},z}$ was modeled (1990-2015). Soil depths below 1 m remained above `r round(psi.1.7.min$median, 2)` MPa (`r round(as.numeric(psi.1.7.min$q97.5), 2)`, `r round(as.numeric(psi.1.7.min$q2.5), 2)`;  median, 95% CI) through all the dry seasons and drought periods, which is above ($\mathrm{\Psi}$~crit~) of the most sensitive tree species (-0.28 MPa).
...
\
\   We found a strong scaling relationship between commonly collected 'soft' traits at the community scale (LMA and WSG) and parameters of leaf vulnerability (or percent loss of conductance, PLC) curves for a few species ($n$ = `r length(sp.exp.param$sp)`), a 'hard' trait. This allowed us to scale-up the leaf vulnerability curves at the community scale ($n$ = `r length(ab.table$Species)` species, see Table S\@ref(tab:ab-param) for model predicted parameters). WSG alone explained `r round(b.wsg.r2*100,0)`% of the variance in $B$ ($p$ `r  b.wsg.p`). We find that leaf conductance declined more rapidly with declining $\mathrm{\Psi}$~leaf~ for species with higher values of WSG and LMA (Fig. S\@ref(fig:obs-plc-curves)). Whether such scaling relationships between hard and soft traits hold true for other forests, and their wider applicability, remains to be seen as data on PLC curves are limited in tropical forests, but supportive and counter examples both exist [@McCulloh.2011; @Hoffmann.2011].

\
\   Another important piece of information for understanding effective rooting depth is stable water isotopes of xylem water and below-ground sources, in the vadose zone and water table. Although the interpretation of these data is still under research [@Deurwaerder.2020], they are critical for model calibration or validation. We lacked data on water table levels which would have been useful to constrain water availability dynamics in the unobserved soil layers and the spatial variability within the plot. As in @Chitra-Tarak.2018, we also interpret our ERD estimates as revealing relative differences among species' effective rooting depths rather than in absolute terms.

\
\   As our ERD model assumes diurnal maximum $\mathrm{\Psi}$~leaf~ as equivalent to $\mathrm{\Psi}$~soil~, ignoring the buffering effect of stem water storage capacitance, this approach may overestimate ERD in species where stem water storage capacitance is important as a drought-avoidance strategy, that is, the group of deciduous species [Borchert.2005jma].

### Future directions

The relationships that we identify between above- and below-ground traits, the water environment vertically below-ground and mortality rates are important for representing diversity in Dynamic Global Vegetation Models, which intrinsically rely on the parameterization of contrasting life history strategies [@Scheiter:2013] and the simulation of competition between those strategies. We found that LMA and WSG were strong predictors of PLC curves for leaves, but not for stems (not shown). This offers the promise of a greater ability to parameterise the 'hard' hydraulic traits with the abundant soft trait data, thus allowing much better representation of forest hydrodynamics. The relationships between ERD and above-ground hydraulic traits that we find will provide an important insight on how to model rooting depths and their coordination or trade-offs with other traits, including hydraulic vulnerability, in order to better represent the functional diversity of tropical forests, and their trajectories into the future.

\
\   Our inverse ERD model was parameterized on 5-year growth data, with five data points over a 25-yr period, which decouples climate events and demographic outcomes. Future studies could better constrain the ERD model with higher resolution growth data such as those from dendrometer bands. However, trees' stem diameter change is a sum total of not just growth, the addition of wood, but also changes due to reversible stem dehydration (xylem, phloem and bark), the importance of which increases with temporal resolution [@Chitra-Tarak.2016; @Chitra-Tarak.2015]. Use of dendrometer band data would thus entail accounting for reversible stem dehydration [@Zweifel:2018], but may also be an avenue to account for stem water storage and temporal dynamics of ERD, a limitation in our current approach. Three of our exploratory ERD models included seasonality in leaf area fraction, but we did not select them as they worsened the fit with growth data for many species (Fig. S\@ref(fig:other-erd-mods)). We interpret this result as that VPD and PLC curves may be adequate to explain inter-census differences in growth, but also acknowledge that our estimates of seasonality of LAI is a tentative estimate combining leaf fall data and the timing of leaf-gain backtracked from leaf lifetime, and omits inter-annual variation. Future studies could further develop models of LAI seasonality and make use of data from new technologies such as drone based monitoring of LAI in species-rich tropical forests [@Park.2019]. This would also open the possibility of testing, whether deciduous species are more shallow rooted than evergreen species as generally observed [@Fan:2017].

\
\   In forests such as BCI where understory tree growth is more limited by availability of light rather than water, inversely estimating ERD for smaller tree size classes, especially from a few growth data points, may need accounting for variation in trees' light environment over time and among trees of the same size class, an issue we minimized by limiting ERD estimates to canopy trees. In sites where water does not limit growth, it may not be possible to parametarize ERD models from growth dynamics, and other avenues such as sap-flux or leaf water-potential dynamics could be explored [@Brum.2017].

\
\   While our ERD model empirically predicts growth via $\mathrm{\Psi}$~leaf~ from $\mathrm{\Psi}$~soil~ at a specific depth, models that mechanistically account for $\mathrm{\Psi}$ through soil, the entire rooting profile, as well as, $\mathrm{\Psi}$~stem~ and $\mathrm{\Psi}$~leaf~, accounting for water content and accumulation of embolised tissues, and then downstream photosynthetic pathways and non-water limiting processes influencing growth are likely to be more accurate [@Christoffersen:2016kd2d; @Sperry.1998]. Although data needs for parameterizing such models [@Christoffersen:2016kd2d; @Sperry.1998] could be greater (e.g. retention curves and capacitance for roots, stems and leaves), adding a degree of parameter uncertainty in community-wide application, such models hold greater promise in improving our understanding of plant physiology [e.g. see @Johnson:2018fad]. 
...
\
Figure \@ref(fig:erd-iso): Modelled Effective Rooting Depth (ERD; mean $\pm$ 1SE; m) versus dry season stable isotopic concentration $\mathrm{\delta}$^2^H~xylem~ (mean $\pm$ 1SE; ‰) for `r length(unique(erd.iso$sp))` canopy species from Barro Colorado Island. Data from [@Meinzer:1999]. See Table  \@ref(tab:ab-param) for the full form of scientific names.

\
Figure \@ref(fig:sp-erd): Modelled Effective Rooting Depth (ERD; mean $\pm$ 1SE in m) for `r  length(erd.sp)` large, canopy species of Barro Colorado Island. Each species' ERD is a median of the species' ERDs (depths) obtained from all hydrological realisations, in which soil layers are vertically resolved into exponentially increasing depths. Hence the non-continuous nature of the ERD axis. 

\
Figure \@ref(fig:erd-iso): Matrix for pairwise correlations (circles) among Effective Rooting Depth (ERD) and hydraulic properties for `r nrow(cor.data)` canopy species found on Barro Colorado Island (Panama). Each circle is color coded by the Spearman's $r$ with red shades for positive $r$ values, while blue shades for negative values, respectively. $p$-values are given only when they are $\ge$0.05. Units are as follows: ERD in m, K~max~ in mmol m^-1^ s^-1^ MPa^-1^), all variables with $\mathrm{\Psi}$ in MPa.

\
Figure \@ref(fig:erd-mort-census-evg): Mortality rate (mean $\pm$ 1SE) versus Modelled Effective Rooting Depth (ERD; mean $\pm$ 1SE in m) for `r nrow(mrate.depth.mean %>% subset(deciduous == "E"))` evergreen, canopy species found on Barro Colorado Island over seven census intervals (1981-2015). $R$^2^ and $p$-values for linear model fits are given in panel insets. Model fits (blue lines) with SE (gray area) are shown for significant fits at $\alpha$ = 0.1.

\
Figure \@ref(fig:erd-time): Modelled species Effective Rooting Depth (ERD; mean $\pm$ 1SE in m) versus time spent below critical hydraulic threshold--the number of days for which $\mathrm{\Psi}_{\textrm{soil},z}$ was below $\mathrm{\Psi}$~crit~, defined as $\mathrm{\Psi}$~80,leaf~, over 1990-2015, where $z$ is the soil depth matching species ERD--for `r length(erd.mrate.sp)` canopy species found on Barro Colorado Island. Each circle represents a species. Circles are colored and slightly jittered horizontally for clarity.
...
```{r other-erd-mods, eval = TRUE, echo = FALSE, message = FALSE, fig.align = 'center', out.width = "80%", fig.pos = 'H', results = "asis", fig.cap = paste0("\\label{lab-other-erd-mods}Effective Rooting Depth versus $\\mathrm{\\delta}^2H$ for each of the ERD models tested (panels). For each ERD model only those species have an estimate of ERD that have passed the test of Pearson's $r$ > 0 and $R$^2^ > 0.5 during ERD parameterisation. The number of species with an ERD estimate as well as $\\mathrm{\\delta}^2H$. or each model are shown in inset, along with the goodness-of-fit ($R$^2^) and $p$-values for a linear relationship between ERD and $\\mathrm{\\delta}^2H$. ERD models in panels A-E correspond to Eq. \\@ref(eq:M1)-\\@ref(eq:M6), respectively.")}
