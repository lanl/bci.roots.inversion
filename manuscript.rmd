...
suppressPackageStartupMessages(
  pacman::p_load(bookdown, knitr, pander, citr))
...
- Deep water access (WA~Deep~) is believed to be an important drought-tolerance mechanism, albeit little studied. Vulnerability to embolism ($\Psi_{88, Stem}$) and safety margins ($\Psi_{min} - \Psi_{88, Stem}$) are shown to be predictors of drought-induced mortality risk, but deep water access may delay plants from reaching these critical thresholds. Here we test its role in drought-tolerance strategies of a tropical forest community (Panama) with a novel data-model approach.
...
- We find that deeper ERD is associated with higher hydraulic conductivity, greater vulnerability to xylem embolism, narrower safety margin, but lower mean mortality rate (*R*^2^ = 0.2, *P* = 0.0xx). Deeper ERD species spent less time below critical hydraulic thredholds indicating that ERD buffers drought-stress.  
...
# Introduction
\
Intensifying droughts [ref] threaten tropical forests, a biome that plays a disproportionately large role in the global carbon and energy cycle, and supports half of the global biodiversity [@Bonan:2008; @Wright:20050th]. Tropical forests are considered an especially drought-vulnerable biome given their narrower range of safety margins [@Choat:2012; @Oliveira:2019], and widespread drought-induced mortality has already been observed across the tropics, especially for large and light-wooded trees [@Bennett:2015; @Chitra-Tarak:2018; @Hilker:2014; @Phillips:2010]. However, state-of-the-art dynamic global vegetation models (DGVMs) fail to capture drought-induced vegetation dynamics in the tropics [@Galbraith:2010; @Powell:2013], as underlying mechanisms of drought-tolerance are not yet fully understood, while data are limited.

\
Among all the mechanisms plants employ to deal with drought, deep water access is arguably the most commonly invoked, but under-studied mechanism. Lack of data on trees rooting or water-sourcing depths has been a key bottlneck, followed by data on water-availability at those depths. A recent meta-analysis shows that maximum rooting depths are known for less than 1% of the World’s 45-53 thousand tree species [@Fan:2017; @Silk:2000]. Further, only a small fraction of those are tropical, even though ~95% of the World’s tree diversity resides in the tropics [@Silk:2000]. Tracer techniques like stable isotopes provide an indirect way of trees' water sourcing depths in which isotopic composition of water in the non-transpiring plant tissue is matched with corresponding gradient in the vertical soil depth, but such data is also available only for ~0.05% of World's tree species [@Evaristo:2016]. Rooting or water-sourcing depths are especially challenging to obtain at community-scale for species-rich tropical forests.

\
Traditionally, species are thought to employ three general mechanisms to deal with drought via a variety of associated traits: (1) avoidance--dry season dormancy via deciduous leaf-habit, (2) delay--greater water access via deep rootedness or valley/riparian habitat preference; or reduced water loss via water storage and stomatal control, and (3) physiological tolerance--continued physiological function at low leaf water potential (Poorter 2008). Recent understanding of 

- traits in global change
- trait-demography are traits really functional (link with plant performance and fitness)
Understanding and explaining the belowground mechanisms and trade‐offs that drive variation in root traits, resource acquisition and plant performance across species
  
- Safety-efficiency trade-off in hydraulic architecture--why deep rooted trees may be both acquisitive with unsafe xylem, yet an overall safe strategy--so long as their high resource environment is stable. 
- consequence on performance
- the problem of large trees--large tree mortality in the tropics

\
In this study, ….growth PLC and water ...This study x times increased the mean number of Percent Loss of Hydraulic Conductivity data reported for or tropical forests on by xx %

\
We test the hypotheses that: in this hyperdiverse forest, interspecific variation in effective rooting depths of canopy trees aligns with a conservative-acquisitive trade-off axis such that trees with deeper effective rooting depths rely on a relatively stable, larger water resource and are thus associated with acquisitive hydraulic architecture: with higher maximum stem hydraulic conductance, but greater xylem vulnerability to embolism and narrow hydraulic safety margins. Despite having a drought-vulnerable xylem, in this humid forest availability of a reliable water at depth allows trees with deeper effective rooting depths to escape the meteorological drought, resulting in lower mortality rates compared to trees with shallow effective rooting depths.

# Materials and Methods
\
In the following sections we describe the study site, the models and data used to estimate and evaluate (1) the soil matric potential dynamics by depth over the forests’ rooting zone, (2) species-specific percent loss of leaf conductivity curves (PLC), and (3) species-specific effective rooting depths. These are followed by analyses to address the proposed hypotheses. Note that only new data on leaf physiological traits collected during this study are described below, details of all other datasets employed are described in the Supplementary Information. 
...
\
To parameterise the model, we identified eleven parameters relevant for in eq (1), determined their ranges based on literature for the study site or for the tropics (Table 1), and sampled 10, 000 ensembles from this parameter space with Latin Hypercube Sampling.

\
We ran ELM-FATES for these 10, 000 ensembles with hourly climate drivers from 1985-2018 initialised with the stand structure of the 50 ha plot in a single site mode (0.1 degree x 0.1 degree centered at 9.1530 $^\circ$N, 280.2539 $^\circ$E). As our key interest here was in soil water availability rather than forest dynamics, we ran ELM-FATES in simplified modes: static stand structure and without plant hydrodynamics, and with a single Plant Functional Type (PFT) of evergreen trees. The latter choice was because only 9.7% of BCI crown area is dry-season deciduous and addition of dry-deciduous PFT did not significantly alter results (not shown).

### Model evaluation
\
We calibrated the model against three key fluxes in Eq. (1), namely, (i) evapotranspiration ($E_{v} + E_{g}$) from the eddy flux tower, (ii) stream discharge for the Conrad Catchment ($q_{over} + q_{drai}$) and (iii) Volumetric Water Content (VWC) obtained with Time Domain Reflectometry (TDR) at three depths (10, 40 and 100 cm; relating to $W_{liq, z}$) at point location as well as opportunistic dry season VWC measured over those depths for the entire 50-ha plot (Figure \@ref(fig:hydro-calib). Among the 10,000 parameter ensembles we chose 100 ensembles that best-fit all three fluxes simultaneously (add details...that is those that maximised the sum of regression coefficients between observed and simulated variables.) See [Table 1](#param) for best-fit parameter ranges.
...
## Model for Percent Loss of Leaf Conductivity

```{r comm-plc-curves, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "30%", fig.pos = 'H', fig.cap = "\\label{lab-comm-plc-curves}Modelled 'vulnerability curves' for standardized *K*~Leaf~ vs. $\\Psi$~Leaf~ for 189 tree species from Panama, color coded by Wood Specific Gravity (units)."}
knitr::include_graphics(file.path("figures/PhenoDemoTraitsPsi/kmax_by_psi/Leaf/std.k.spkmax_by_psi_color_by_Specific Gravity_predicted_AB.jpeg"))
```

See Figure \@ref(fig:comm-plc-curves) 
...
To test whether ERD plays a role in alleviating mortality risk, we regressed species ERD against mean mortality rates over the 33 year record (1985-2015). To assess whether this may be due to deeper ERD species escaping the experience of catastrophic soil drought, we calculated the proportion of days over the study period (1990-2015) for which soil water potentials in the soil zone matching species ERD (𝚿ERD) were below species P80, leaf.  
...
```{r sp-erd, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "60%", fig.pos = 'H', results = "asis", fig.cap="\\label{lab-sp-erd}Modelled Effective Rooting Depth (ERD; mean $\\pm$ (add error) in m) for 32 large, canopy species of Barro Colorado Island. See Table S2 for scientific names of species codes"}
knitr::include_graphics(file.path("figures/PhenoDemoTraitsPsi/ERD_by_sp_large_canopy.jpeg"))
```


```{r erd-hyd, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "60%", fig.pos = 'H', results = "asis", fig.cap ="\\label{lab-erd-hyd}Modelled Effective Rooting Depth (ERD; mean $\\pm$ (add error) in m) versus hydraulic propertiese for seven large, canopy species found on Barro Colorado Island. *R*^2^ and *P*-values are given in panel insets."}
...
\
Species with deeper ERD showed lower mean mortality rates across the 33 year record (1982-2015; *R*^2^ = 0.13, *P* = 0.029, *n* = xx; Figure \@ref(fig:erd-mort)). For five out of seven census intervals, species with shallower ERD had significantly higher mortality anomalies (MortalityCensus Interval, sp - Mean(MortalityCensus Interval, sp) than deeper ERD species; Figure (\@ref(fig:erd-mort-anom)). Analysis of time spent under catastrophic soil drought showed that the proportion of days species $\Psi_{z = ERD}$ was more negative than species $\Psi_{80, Leaf}$ decreased with species ERD (Figure \@ref(fig:erd-time)). Species with ERD deeper than 1 m did not cross species $\Psi_{80, Leaf}$ at all for the entire 25 year period for which soil water potential was modelled (1990-2015). 
...
```{r obs-plc-curves, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "40%", fig.pos = 'H', results = "asis", fig.cap = "\\label{lab-obs-plc-curves} 'vulnerability curves' fitted to observed data shown as standardized *K*~Leaf~ vs. $\\Psi$~Leaf~ for 21 tree species from Panama, color coded by Wood Specific Gravity (units)."}
knitr::include_graphics(file.path("figures/PhenoDemoTraitsPsi/kmax_by_psi/Leaf/std.k.commkmax_by_psi_color_by_Specific Gravity_predicted_AB.jpeg"))
```

```{r ab-fit, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "60%", fig.pos = 'H', fresults = "asis", fig.cap = "\\label{lab-ab-fit}Parameters of 'vulnerability curve' (Eq.) fitted to data versus predicted from soft traits (Eq.)"}
...
| 5 | fates_rootb_ par | CLM rooting distribution parameter | 0.217 - 1.956  per m | Parameter b in Eq. 2 in (Zeng 2001) that regulates the depth of the rooting profile. Chosen range of *b* is derived using this equation so as to fit the observed range of rooting depth (d~r~) of 2 - 18 m for Tropical Deciduous Forest (mean ± Standard Error (SE); 3.7 ± 0.5, *n* = 5 trees; min = 2, max = 4.7)  and Tropical Evergreen Forest (mean $\pm$ S.E.; 7.3 $\pm$ 0.5, n = 3 trees and 3 communities; min = 2, max = 18 m) combined (Canadell et al. 1996). Besides the direct observation of roots at 18 m included by (Nepstad et al. 1994) in Paragominas, eastern Amazonia that is included in the above study; in Tapajos, eastern Amazonia water extraction by roots was also inferred up to 18 m. (Davidson et al. Forest Science 2011) |
...
| 11 | fpi_max | Maximum interception fraction of precipitation | 0.05-0.44 | Based on throughfall data from (Zimmermann et al. 2010) and precipitation data for BCI from STRI Physical Monitoring program, defined for precipitation events greater than 10 mm. | 
