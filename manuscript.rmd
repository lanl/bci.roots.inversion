...
(\#eq:erd)
\end{equation}
where $L$ is the likelihood, $H$ = {1, .., 100}, refer to the 100 hydrological realisations and $Z$ = {0.4, `r paste(soil.depths[8:length(soil.depths)], collapse=", ")`} m. We removed finely resolved soil layers below 0.21 m, averaged $\Psi_{\textrm{soil},z}$ for depths 0.21, 0.37 and 0.62 at 0.4 m. 
...
We obtained leaf hydraulic vulnerability curves ($K_\textrm{leaf}$ vs. $\Psi$~leaf~; see Table \@ref(tab:symbols) for trait symbols and their definitions) for adult trees of 21 common species from Zailaa et al. (to be submitted; see SI for brief description of methods) described as:
...
Xylem sap $\mathrm{\delta}$^2^H values can provide an index of rooting depth, which we used to validate the ERD models as an independent observation of ERD. We used xylem sap $\mathrm{\delta}$^2^H values from BCI given in @Meinzer:2001 for the dry-season of March 1997 as this period showed largest seasonal divergence among species, with a corresponding vertical gradient of $\mathrm{\delta}$^2^H in the soil. (Fig. S\@ref(fig:iso-data)). Briefly, this isotopic approach makes use of a naturally formed gradient in deuterium to hydrogen ratio of soil water vertically belowground, which is generally formed due to evaporative enrichment (here, during the dry season). Shallow soil water has a greater deuterium to hydrogen content than deep soil water (here, $\mathrm{\delta}$^2^H ranged from -16.5 to -50 ‰ over 0.05 to 1 m depth), with ground-water having the lowest deuterium to hydrogen content (here, -60 ‰), typically reflecting un-fractionated rain water. As root water-uptake is generally a non-fractionating process, $\mathrm{\delta}$^2^H of water in the xylem sap of a tree reflects the signature of the source water. Thus water-sourcing depth of the tree can be identified as the depth with the same value of $\mathrm{\delta}$^2^H as that of the xylem sap, with the uncertainty that the value of $\mathrm{\delta}$^2^H may also result from a mixing of water from shallower or deeper depths [@Dawson.1991]. Analyses of xylem sap $\mathrm{\delta}$^2^H of the 12 species studied by @Meinzer:2001 showed that four species sourced water between 1 m and ground water. Two other had water access at least 0.8 m deep, one was restricted to water access between 0 to 0.2 m and five fell in the range over which the relationship between soil depth and soil water $\mathrm{\delta}$^2^H was variable in the 0.2 to 0.7 m depth range. 

\   We compared six species with data for $\mathrm{\delta}$^2^H as well as modelled estimates of ERD, after removing those species in @Meinzer:2001 that are likely to have no leaves in March 1997 because xylem sap $\mathrm{\delta}$^2^H of these species may not be linked with water sourced at the time of measurement and thus may not be comparable with other species that had leaves. We also removed *Guapira standleyana*, whose xylem sap isotope value was not well constrained by the soil water isotope depth profile. For each model of ERD (Eq. \@ref(eq:M-chosen) and Eqs. in SI), estimated species ERD was regressed against species xylem sap $\mathrm{\delta}$^2^H (‰).

## Relationship of ERD with above-ground hydraulic traits

To evaluate whether ERD was associated with above-ground hydraulic traits ($n$ = 8 species with both datasets), we regressed species ERD against maximum stem hydraulic conductance ($K_\textrm{max,stem}$), leaf turgor loss point ($\Psi$~tlp~), vulnerability to embolism from cavitation measured in terms of pressure at which 88% of stem hydraulic conductivity is lost ($\Psi$~88,stem~), and hydraulic safety margins ($\Psi$~min~ - $\Psi$~88,stem~). See [SI](#SIData.brett) for data collection and estimation of these variables.
...
\   Soil Water Potential dynamics ($\Psi_{\textrm{soil},z}$) by depth $z$ (from 0.1 through 13 m) obtained from the best-fit simulations revealed that every 5-yr census interval had an extreme hydrological drought year, but census intervals varied in terms of number of such extreme years, drought intensity at depth, drought seasonality and duration (Fig. \@ref(fig:swp-droughts)). Notably, soil droughts in census intervals 2000-2005 and 2005-2010 distinctly occurred in the wet season, with one to two extreme soil drought years each, depending on depth location (Fig. S\@ref(fig:swp-droughts)). Across the simulation period (1990-2015) $\Psi_{\textrm{soil},z}$ remained above -0.5 MPa from depths 1.7 to 13 m in the simulations (Fig. S\@ref(fig:swp-all-depths)).
...
\
\   Our inverse ERD model was parameterized on 5-year growth data, with five data points over a 25-yr period, which decouples climate events and demographic outcomes. Future studies could better constrain the ERD model with higher resolution growth data such as those from dendrometer bands. However, trees' stem diameter change is a sum total of not just growth, the addition of wood, but also changes due to reversible stem dehydration (xylem, phloem and bark), the importance of which increases with temporal resolution [@Chitra-Tarak.2016; @Chitra-Tarak.2015]. Use of dendrometer band data would thus entail accounting for stem hydrostatic flexing [@Zweifel:2018], but may also be an avenue to account for stem water storage and temporal dynamics of ERD, a limitation in our current approach. Three of our exploratory ERD models included seasonality in leaf area fraction, but we did not select them as they worsened the fit with growth data for most species (Fig. S\@ref(fig:other-erd-mods)). We interpret this result as that VPD and PLC curves may be adequate to explain growth, but also acknowledge that our estimates of seasonality of LAI is a tentative estimate combining leaf fall data and the backtracked timinng of leaf-gain from leaf life-time, and omits inter-annual variation. Future studies could further develop models of LAI seasonality and make use of data from new technologies such as drone based monitoring of LAI in species-rich tropical forests [@Park.2019]. In forests such as BCI where understory tree growth is more limited by availability of light rather than water, inversely estimating ERD for smaller tree size classes, especially from a few growth data points, may need accounting for variation in trees' light environment over time and among trees of the same size class, an issue we minimized by limiting ERD estimates to canopy trees. In sites where water does not limit growth, it may not be possible to parametarize ERD models from growth dynamics, and other direct avenues such as sap-flux dynamics could be explored. To estimate topographic variation in ERD within or across species, future studies may use soil moisture dynamics from a distributed hydrological model [e.g. @Schwantes:2018]. We note that data availability on soil water retention curve ($\Psi$~soil~ vs. VWC), hydraulic conductivity (K~soil~) by depth, soil moisture by depth, stream discharge and evapotranspiration were important for effective calibration of our 1-D hydrological model. We recommend wide-spread and coordinated collection of these variables as well as water-table levels in forest-inventory sites to allow for estimation of trees' water environments. While our model empirically predicts growth via $\Psi$~leaf~ from $\Psi$~soil~ at a specific depth, models that mechanistically account for $\Psi$~soil~ and $\Psi$~root~ over the entire rooting profile and then $\Psi$~stem~ and $\Psi$~leaf~, accounting for water content and embolised tissues, and downstream photosynthetic pathways and non-water limiting processes are likely to be more accurate in predicting growth [@Christoffersen:2016kd2d; @Sperry.1998]. Although data needs for parameterizing such models [@Christoffersen:2016kd2d; @Sperry.1998] could be greater (e.g. retention curves, capacitance for roots, stems and leaves), which increases parameter uncertainty in community-wide application, such models hold greater promise in improving our understanding of plant physiology [e.g. see @Johnson:2018fad].

# Conclusions

Establishing relationships between environment, traits and demographic outcomes of plants is imperative for developing a predictive plant ecology. Trees realized water environments and water-access are nonetheless rarely studied. To our knowledge this is the first study to demonstrate a mechanistic link between trees' realized drought experience, above-ground hydraulic architecture and long-term mortality outcomes for a species-rich forest. We report here that deep-water access plays a role in alleviating mortality of hydrualically vulnerable trees. This has important implications for our predictive understanding of tropical forest dynamics under current and future climate, and suggests that greater emphasis should be given to predictive modeling of tree root allocation and distribution (e.g. @Wijk.2001) as well as trees' hydrological environments. Our community-scale framework for modeling effective rooting depths and leaf vulnerability curves indicate the possibilities in expanding the use of these critical, hard-earned observations in species-rich forests towards community-scale generalizations.
