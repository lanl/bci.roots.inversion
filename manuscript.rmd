...
- Deep water access (WA~Deep~) is believed to be an important drought-tolerance mechanism, but under-studied. Vulnerability to embolism ($\Psi_{88, Stem}$) and safety margins ($\Psi_{min} - \Psi_{88, Stem}$) are shown to be predictors of drought-induced mortality risk, but deep water access may delay plants from reaching these critical thresholds. Here we test its role in drought-tolerance strategies of a tropical forest community (Panama) with a novel data-model approach.
...
Trees' form and function are outcome of the environment in which trees have evolved. And as cliamte changes  and Traits link to plant environment - trait - performance 
- traits in global change as environment changes
...
Differential dynamics of water environment by depth 
...
\
In this study, what we do differently: which soil depth dynamics link dynamics of water dynamics in whole-soil column, atmospheric drought via
...This study increased the mean number of tropical forest species with data on Percent Loss of Hydraulic Conductivity data by xx %
...
```{r comm-plc-curves, eval = TRUE, echo = FALSE, message = FALSE, fig.align ='center', out.width = "30%", fig.pos = 'H', fig.cap = paste("\\label{lab-comm-plc-curves} Modelled standardized 'Vulnerability curves' for  $K_{x, Leaf}$ vs. $\\Psi_{x, Leaf}$ at community-scale based on communit-scale data on $WSG$ & $LMA$ and their relationships with vulnerability curve parameters measured for a few species. Each line represents a species, color coded by $WSG$.")}
knitr::include_graphics(file.path("figures/PhenoDemoTraitsPsi/kmax_by_psi/Leaf/kmax_by_psi_color_by_Wood Specific Gravity_predicted_AB.jpeg"))
```
